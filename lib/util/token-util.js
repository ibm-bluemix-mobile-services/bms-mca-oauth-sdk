 ...
 * @param imfCert
 */
function getOAuthAccessToken(appId,clientId,imfCert) {
	var deferred = Q.defer();

	var imfServiceUrl = process.env.imfServiceUrl;
	var requestUrl = imfServiceUrl+"/authorization/v1/apps/"+appId+"/token";

	var requestOptions = {url: requestUrl,headers: {'Authorization': 'IMFCert '+imfCert}, form:{'grant_type':'client_credentials','client_id':clientId}};
 ...
 */
Util.getImfTokenByCertificate = function(imfCert,appId,clientId,uaaCredential) {
	var deferred = Q.defer();

	if (uaaCredential) {
		verifyPermission(appId,uaaCredential).
		then(function(hasPermission){
			return getOAuthAccessToken(appId,clientId,imfCert);
		},function(err) {
